    ...
    for i, structure_info in enumerate(structure_combo_list):
        ...
        for j in range(nr_of_atoms):
            ...
            for combo in all_combos:
                combo = str(combo)
                combo = combo.replace('\'', '').replace(', ', '').replace('(', '').replace(')', '')
                combo = combo.replace(',', '')
        ...
        for substructure in all_subs_comb_list:
            ...
            if mol_sub != None and substructure not in sub_smile_list and substructure != '' and len(substructure) > 1:
        ...
        for sub_mol2 in sub_mol_list:
            ...
            if structure_mol.HasSubstructMatch(sub_mol2) == True and sub_smile3 not in valid_sub_list:
...
def filter5(structure_info, filter4_dict, input_nr_of_bonds, parameter_info):
    ...
    with open("final_perm_test_output.txt", 'w') as db_file:
